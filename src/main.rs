...
struct Position(pub Vector2<f32>);
...
impl System<()> for PongSystem {
	fn run(&mut self, arg: RunArg, _: ()) {
		...
		// Process all planks
		for (ref mut plank, &mut Position(ref mut position), &mut Velocity(ref mut velocity), ref mut local) in (&mut planks.pass(), &mut positions, &mut velocities, &mut locals).join() {
			match plank.side {
		...
		// Process the ball
		for (ref mut ball, &mut Position(ref mut position), &mut Velocity(ref mut velocity), ref mut local) in (&mut balls, &mut positions, &mut velocities, &mut locals).join() {
		...
		for (&mut Position(ref mut position), &mut Velocity(ref mut velocity)) in (&mut positions, &mut velocities).join() {
...
impl State for Pong {
	fn on_start(&mut self, world: &mut World, assets: &mut AssetManager, pipe: &mut Pipeline) {
		...
		// Create a left plank entity
		{
			...
			let pos = Position(Vector2::new(-1.0 + plank.dimensions[0] / 2.0,0.0));
			world.create_now()
				.with(square.clone())
				.with(plank)
				.with(pos)
